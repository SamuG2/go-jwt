...
import (
	...
	"github.com/SamuG2/go-jwt/models"
...
func SignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while registering the user"})
		}
		...
		if tokenErr != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while creating the token"})
		}
		user.Token = &token
